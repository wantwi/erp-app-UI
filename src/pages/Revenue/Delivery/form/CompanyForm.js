...
const validationSchema = Yup.object().shape({
    ...
    currency: Yup.number().required('Currency is required').positive('Currency is required'),
    status: Yup.string(),
...
const CompanyForm = ({ TaxTable, refetch, BusinessPartners, items, rowInfo, setRowInfo, setShowLoading, currenciesList, modal_backdrop, setmodal_backdrop, setMinimized }) => {
    ...
    const { isLoading, isSuccess, mutate } = usePost(`${process.env.REACT_APP_ADMIN_URL}/SalesDeliveries`, "sales-deliveries", onPostSuccess, onError)

    const { isLoading: isPutLoading, isSuccess: isPutSuccess, mutate: putMutate } = usePut(`${process.env.REACT_APP_ADMIN_URL}/SalesDeliveries/${rowInfo?.id}`, 'sales-deliveries', onPutSuccess, onError)
    ...
    function onSubmit(data) {
        ...
        const postData = {
            ...
            taxTable: { id: data?.taxTable?.value, name: data?.taxTable?.label },
            ...
            deliveryItems: data?.entries.map(x => ({
    ...
    useEffect(() => {
        ...
        if (rowInfo?.id) {
            ...
            const details = rowInfo?.deliveryItems.map(x => ({
        ...
        >
            <div className="modal-header">
                <h5 className="modal-title" id="staticBackdropLabel">
                    {rowInfo?.id ? `Edit Transaction No. - ${rowInfo?.id}` : "Add Delivery"}
                </h5>
                <div>
                    ...
                    <div className="row mb-3">
                        ...
                        <div className="col col-md-2">
                            ...
                            <Controller
                                name="taxTable"
                                control={control}
                                rules={{ required: 'Please select a tax' }}
                                render={({ field }) => (
                                    <Select
                                        value={field.value}
                            ...
                            />
                            <div className="invalid-feedback">{errors.taxTable?.message}</div>
                        </div>
                    </div>
