...
const CompanyForm = ({ refetch,
    ...
    const { isLoading, isSuccess, mutate } = usePost(`${process.env.REACT_APP_ADMIN_URL}/SalesOrders`, "sales-orders", onPostSuccess, onError)
    const { isLoading: isDraftLoading, isSuccess: isDraftSuccess, mutate: draftMutate } = usePost(`${process.env.REACT_APP_ADMIN_URL}/SalesOrders/Draft`, "sales-orders", onPostSuccess, onError)

    const { isLoading: isPutLoading, isSuccess: isPutSuccess, mutate: putMutate } = usePut(`${process.env.REACT_APP_ADMIN_URL}/SalesOrders/${rowInfo?.id}`, 'sales-orders', onPutSuccess, onError)
    ...
    function onSubmit(data) {
        ...
        const postData = {
            ...
            salesOrderItems: data?.entries.map(x => ({
                id: 0,
    ...
    useEffect(() => {
        if (isSuccess) {
            ...
            showToast("success", "Delivery Created Successfully", "Notice")
            reset()
            setShowLoading(false)
        }
        if (isLoading) {
        ...
        if (isPutSuccess) {
            // setmodal_backdrop(false)
            showToast("success", "Delivery Updated Successfully", "Notice")
            // reset()
            setShowLoading(false)
        }
        if (isPutLoading) {
        ...
        if (isDraftSuccess) {
            setmodal_backdrop(false)
            showToast("success", "Delivery  Saved as Draft", "Notice")
            // reset()
            setShowLoading(false)
        }
        if (isDraftLoading) {
    ...
    useEffect(() => {
        ...
        if (rowInfo?.id) {
            ...
            const details = rowInfo?.salesOrderItems.map(x => ({
            ...
            >
                <div className="modal-header">
                    <h5 className="modal-title" id="staticBackdropLabel">
                        {rowInfo?.id ? `Edit Transaction No. - ${rowInfo?.number}` : "Add Delivery"}
                    </h5>
                    <div>
                ...
                <form onSubmit={handleSubmit(onSubmit)}>
                    <div className="modal-body">
                        <div className="row mb-3">
                            <div className="col col-md-3">
                                <div >
                                    <label className="form-label">Delivery Date <i className="text-danger">*</i></label>
                            ...
                            <div className="col col-md-3 offset-md-4">
                                ...
                                <div >
                                    <label className="form-label">Delivery Number <i className="text-danger"></i></label>
                                    <Controller
                                        name={`documentNumber`}
                                        control={control}
                                        render={({ field }) => (
                                                        ...
                                                        <Controller
                                                            ...
                                                            render={({ field, fieldState }) => (
                                                                <Select
                                                                    name={field.name}
                                                                    value={field.value}
                                                                    options={TaxTable.map((x) => ({ value: x?.id, label: x?.name }))}
                    ...
                    <div className="modal-footer">
                        ...
                        <button hidden type="submit" onClick={() => setAction(3)} className="btn btn-sm  btn-info"><i className="fas fa-bus me-2"></i>Prepare Delivery</button>
                        <button hidden type="submit" onClick={() => setAction(2)} className="btn btn-sm  btn-primary"><i className="fas fa-save me-2"></i>Save as Draft</button>
