...
const Users = () => {
  ...
  const { refetch, isSuccess: isGetByIdSuccess } = useGetById(`${process.env.REACT_APP_ADMIN_URL}/Project/${selectedRow?.id}`, 'projects', selectedRow?.id, onGetByIdSuccess)

  const { mutate, isSuccess } = useDelete(`${process.env.REACT_APP_ADMIN_URL}/Project/${selectedRow?.id}`, "projects", onDeleteSuccess, onDeletError)
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        Header: 'Start Date',
        accessor: 'startDate',
        Cell: ({ value }) => value ? convertDateUSA(new Date(value).toISOString()) : ''
  ...
  //meta title
  document.title = "Biz-360 ERP| Project";
  ...
  const { isLoading, isFetching, data } = useGet(`${process.env.REACT_APP_ADMIN_URL}/Project/GetAllProjects`, "projects", onsuccess, onError)
  ...
  return (
    <React.Fragment>
      ...
      <div className="page-content">
        <Container fluid>
          {/* Render Breadcrumbs */}
          <Breadcrumbs title="Administration" breadcrumbItem="Project" />
