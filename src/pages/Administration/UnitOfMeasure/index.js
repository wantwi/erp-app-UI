...
const Users = () => {
  ...
  const { refetch, isSuccess: isGetByIdSuccess } = useGetById(`${process.env.REACT_APP_ADMIN_URL}/UnitsOfMeasure/${selectedRow?.id}`, 'units-measure', selectedRow?.id, onGetByIdSuccess)

  const { mutate, isSuccess } = useDelete(`${process.env.REACT_APP_ADMIN_URL}/UnitsOfMeasure/${selectedRow?.id}`, "units-measure", onDeleteSuccess, onDeletError)
  ...
  //meta title
  document.title = "Biz-360 ERP| Unit of Measure";
  ...
  const { isLoading } = useGet(`${process.env.REACT_APP_ADMIN_URL}/UnitsOfMeasure/GetAll`, "units-measure", onsuccess, onError)
  ...
  return (
    <React.Fragment>
      ...
      <div className="page-content">
        <Container fluid>
          {/* Render Breadcrumbs */}
          <Breadcrumbs title="Administration" breadcrumbItem="Unit of Measure" />
          ...
          {/* Modal */}
          <PageForm baseData={dataList} rowInfo={rowInfo} setShowLoading={setShowLoading} modal_backdrop={modal_backdrop} setmodal_backdrop={setmodal_backdrop} setMinimized={setMinimized} />
