...
const validationSchema = Yup.object().shape({
  ...
  parentId: Yup.number(),//.required('Parent Cost Center is required'),
...
function PageForm({ rowInfo, data: parentCenters, setShowLoading, modal_backdrop, setmodal_backdrop, setMinimized }) {
  ...
  const onPutsuccess = (data) => {
    showToast("success", "Cost Center Updated Successfully", "Notice")
  ...
  const { mutate, isSuccess, isLoading } = usePost(`${process.env.REACT_APP_ADMIN_URL}/CostCenter`, "cost-center", onsuccess, onError)

  const { mutate: putMutate, isSuccess: isPutSuccess, isLoading: isPutLoading } = usePut(`${process.env.REACT_APP_ADMIN_URL}/CostCenter/${rowInfo?.id}`, "cost-center", onPutsuccess, onPutError)
  ...
  useEffect(() => {
    if (isSuccess) {
      ...
      showToast("success", "Cost Center Created Successfully", "Notice")
      formik.resetForm()
      setShowLoading(false)
    }
    if (isLoading) {
    ...
    >
      <div className="modal-header">
        <h5 className="modal-title" id="staticBackdropLabel">{
          rowInfo?.id ? `Edit ${rowInfo?.name}` : "Add Cost Center"
        }</h5>
      ...
      <form onSubmit={formik.handleSubmit}>
        <div className="modal-body">
          ...
          <div className="row">
            <div className="col">
              <div className={`mb`}>
                <label className="form-label">Parent Cost Center <i className="text-danger"></i></label>
                <Select
                  value={selectedGroup}
                  onChange={(value) => {
