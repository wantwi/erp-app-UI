...
const TableContainer = ({
  ...
  return (
    <Fragment>
      <Row>
        ...
        <Col md="4" sm={12}>
          <FormGroup className="">
            ...
            <InputGroup>
              <select className="form-control">
                <option>Select Sub eriod</option>
                {
                  monthsOfYear.map(x => <option key={x}>{x}</option>)
                }
      ...
      <Row className="mb-2">
        <Col md={customPageSizeOptions ? 3 : 2} hidden>
          <select
      ...
      <div className="table-responsive react-table" >
        <div hidden>
          ...
          <div>
            {
              allColumns.map(x => <div key={x?.id}>
                <label>
                  <input id={x?.id} defaultChecked type='checkbox' onClick={toggleHidden} />
                  {x?.Header}
                </label>
              </div>)
            }
